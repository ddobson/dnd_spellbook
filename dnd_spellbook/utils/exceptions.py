...
def dnd_spellbook_app_exception_handler(exc, context):
    ...
    elif isinstance(exc, ValidationError) and exc.detail[0] == const.SPELLBOOK_SPELL_CLASS_VALIDATION_ERROR:
        return Response(
            data={'spells': const.SPELLBOOK_SPELL_CLASS_VALIDATION_ERROR},
            status=status.HTTP_400_BAD_REQUEST,
        )
    else:
